...
- 注册 push 只能使用运行时注入的辅助账号 classic PAT（环境变量 `ACCO_T` 或 JVM 参数 `-Dacco.t=...`，权限选 `repo`），不得使用 fine-grained PAT，不得退回用户本机主 GitHub 账号凭据；不得把辅助账号密码、token 或其它密钥写入代码、文档、提交记录、说明书或配置文件。
- 可以用浏览器协助用户打开 GitHub token 创建页面，但必须使用独立浏览器配置或全新浏览器实例，不复用用户主账号会话；token 的创建、复制和保存必须由用户处理，Agent 不获取、不记录、不提交任何 GitHub 密钥。
- GitHub 注册不能通过打开浏览器网页伪装成 App 功能；App 运行时必须通过 Git push 或直接 HTTP 等真实发包路径完成请求。
- 禁止兜底式编程：一个功能只能有一条明确实现路径，一路走通；不得写备用路径、演示兜底、静默降级、兼容性分支或“失败后换另一套方案”的冗余代码。输入校验、明确报错和必要安全检查不属于兜底，必须保留。
...
- 流程图线条必须横平竖直；必要时使用折线，不使用斜线。
- 流程图框内文字只允许中文描述，或无法避免的英文首字母缩写，例如 `JVM`、`IP`、`UDP`、`ACK`、`SHA`；不得写代码名、方法名、类名或普通英文说明。
- 流程图线条不得交叉。
- 一个节点可以有多个输入或输出，但同一个输入点或输出点不得连接多条线。
- 连线文字必须绑定在线条本身，不得使用和线条分离的独立文本框。
