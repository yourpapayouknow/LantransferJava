...
- 真实 UDP 多线程传输调度与实时进度：目标级并发 UDP 发送、ACK 等待、后台接收、接收目录落盘、后端 FIFO 传输队列、发送端进度快照推送和接收端进度 toast 展示已由 `UdpTx/UdpRx/LocalBackend/FileTransfer/MainWindow` 实现；未实现的是分片级并发。推荐替代方案：只有在大文件性能不足时，再把单文件分片分派到并发 worker，并配套滑动窗口或缺失分片重传协议，避免 UDP 乱序导致误判失败。
